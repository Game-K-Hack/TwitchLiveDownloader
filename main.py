...
from tkinter import PhotoImage, Tk, LabelFrame, StringVar, Label, Entry, Spinbox, Button, filedialog, messagebox
import datetime
import requests
import urllib.parse
import utils
...
def _size(path):
    ...
    size_value.set(utils.humanbytes(s))
    l_size.update()
...
def download(channel_name, id_vid):
    ...
    while is_start:
        ...
        Core.ACCESS_REQUEST["variables"]["login"] = channel_name
        ...
        if "transcode_does_not_exist" in m3u8:
            _status(False)
            is_start = False
